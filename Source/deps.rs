...
use crate::{internal::template, package_manager::PackageManager, utils::colors::*};
use std::process::{Command, Output};

fn is_rustc_installed() -> bool {
	Command::new("rustc").arg("-V").output().map(|o| o.status.success()).unwrap_or(false)
}

fn is_cargo_installed() -> bool {
	Command::new("cargo").arg("-V").output().map(|o| o.status.success()).unwrap_or(false)
}
...
fn is_trunk_installed() -> bool {
	Command::new("trunk").arg("-V").output().map(|o| o.status.success()).unwrap_or(false)
}

fn is_dioxus_cli_installed() -> bool {
	Command::new("dx").arg("-V").output().map(|o| o.status.success()).unwrap_or(false)
}
...
fn is_webkit2gtk_installed(rc: bool) -> bool {
	Command::new("pkg-config")
		.arg(if rc { "webkit2gtk-4.1" } else { "webkit2gtk-4.0" })
		.output()
...
fn is_xcode_command_line_tools_installed() -> bool {
	Command::new("xcode-select").arg("-p").output().map(|o| o.status.success()).unwrap_or(false)
}
...
pub fn print_missing_deps(pkg_manager: PackageManager, template: Template, rc: bool) -> bool {
	...
	let (webkit2gtk_installed, rsvg2_installed) = (is_webkit2gtk_installed(rc), is_rsvg2_installed());
	...
	let (largest_first_cell, largest_second_cell) =
		missing_deps.iter().fold((0, 0), |(mut prev_f, mut prev_s), (f, s)| {
			let f_len = f.len();
			if f_len > prev_f {
				prev_f = f_len;
			}
