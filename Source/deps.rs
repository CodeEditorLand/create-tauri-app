...
fn is_cli_installed(cli:&str, arg:&str) -> bool {
	Command::new(cli).arg(arg).output().map(|o| o.status.success()).unwrap_or(false)
}
...
/// Print missing deps in a table and returns whether there was any missing
/// deps.
pub fn print_missing_deps(
	...
	if !missing_deps.is_empty() {
		println!(
			"\n\nYour system is {YELLOW}missing dependencies{RESET} (or they do not exist in \
			 {YELLOW}$PATH{RESET}):"
		);
