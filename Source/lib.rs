...
where
	...
	// tauri.conf.json and if valid, it will also be used in Cargo.toml,
	// Package.json ...etc
	...
	let category = if manager.is_none() {
		...
		// Skip prompt, if only one category is detected or explicit skip requested by
		// `-y/--yes` flag
	...
	let pkg_manager = match manager {
		...
		None => {
			if let Some(category) = category {
				...
				// Skip prompt, if only one package manager is detected or explicit skip
				// requested by `-y/--yes` flag
	...
	// then almost all prompts are skipped so we need to make sure that the
	...
	if has_missing {
		...
		println!(
			"Make sure you have installed the prerequisites for your OS: \
			 {BLUE}{BOLD}{prereqs_url}{RESET}, then run:"
		);
	} else {
