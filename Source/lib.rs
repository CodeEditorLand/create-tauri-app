	...
	A: Into<OsString> + Clone,
{
	...
	let args::Args { skip, mut mobile, no_mobile, beta, manager, project_name, template, force } =
		args;
	...
	if (mobile.is_some() || no_mobile.is_some()) && !beta {
		let flag = if mobile.is_some() { "--mobile" } else { "--no-mobile" };
	...
	let project_name = match project_name {
		...
		None => {
			if skip {
				defaults.project_name.context("default project_name not set")?
			} else {
	...
	// Render the template
	template.render(&target_dir, pkg_manager, &project_name, &package_name, beta, mobile)?;
	...
	if target_dir != cwd {
		println!(
			"  cd {}",
			if project_name.contains(' ') { format!("\"{project_name}\"") } else { project_name }
		);
	}
...
fn to_valid_pkg_name(project_name: &str) -> String {
	...
	let ret = ret.chars().skip_while(|ch| ch.is_ascii_digit() || *ch == '-').collect::<String>();
