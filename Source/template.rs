...
impl<'a> Template {
	...
	pub fn render(
		...
		let manifest_template_data: HashMap<&str, &str> = [
			...
			("double_dash_with_space", if pkg_manager == PackageManager::Npm { "-- " } else { "" }),
		]
		.into();
		...
		let template_data: HashMap<&str, String> = [
			...
			(
				"dev_path",
				lte::render(manifest.dev_path.unwrap_or_default(), &manifest_template_data)?,
			),
			(
				"dist_dir",
				lte::render(manifest.dist_dir.unwrap_or_default(), &manifest_template_data)?,
			),
