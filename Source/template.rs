...
impl<'a> Template {
	...
	pub fn render(
		...
		let mut template_data:HashMap<&str, String> = [
			...
			("project_name_pascal_case", project_name_pascal_case.to_string()),
		...
		let write_file = |file:&str, template_data| -> anyhow::Result<()> {
			// remove the first component, which is certainly the template directory they
			// were in before getting embeded into the binary
			...
			let file_name = match &*file_name {
				...
				// flags are supported package managers, and `v-$versionNumber` (tauri version
				// filter). example: "%(pnpm-npm-yarn-stable-v1)%package.json"
