...
// why was this module named `lte`? short for "Light Template Engine" and might
// be extracted into its own crate.
...
impl<T:AsRef<str>> From<T> for TemplateParseError {
	fn from(value:T) -> Self { Self { message:value.as_ref().to_string() } }
}
...
impl Display for Token<'_> {
	fn fmt(&self, f:&mut std::fmt::Formatter<'_>) -> std::fmt::Result {
		match self {
			...
			Token::If => write!(f, "if"),
			Token::Var(var) => {
				write!(f, "{} (variable)", String::from_utf8_lossy(var))
			},
...
const KEYWORDS:&[(&[u8], Token)] =
	&[(b"if", Token::If), (b"else", Token::Else), (b"endif", Token::EndIf)];
...
impl<'a> Lexer<'a> {
	fn new(bytes:&'a [u8]) -> Self {
		let len = bytes.len();
		Self { len, bytes, cursor:0, in_bracket:false }
	}

	fn current_char(&self) -> char { self.bytes[self.cursor] as char }

	fn next_char(&self) -> char { self.bytes[self.cursor + 1] as char }
...
impl<'a> Parser<'a> {
	fn new(tokens:&'a [Token<'a>]) -> Self {
		Self { len:tokens.len(), tokens, cursor:0 }
	}

	fn current_token(&self) -> Token<'a> { self.tokens[self.cursor] }

	fn skip_brackets(&mut self) {
		if self.cursor < self.len {
			while self.current_token() == Token::OBracket
				|| self.current_token() == Token::CBracket
			{
				self.cursor += 1;
	...
	fn consume_if(&mut self) -> Result<Option<Stmt<'a>>> {
		if self.current_token() == Token::If {
			...
			Ok(Some(Stmt::If { var, negated, condition, else_condition }))
		} else {
			Ok(None)
		}
	}
	...
	fn next(&mut self) -> Result<Option<Stmt<'a>>> {
		...
		if let t @ Token::Invalid(..) = self.current_token() {
...
mod tests {
	...
	fn it_performs_condition() {
		let template =
			"<html>Hello {% if alpha %}alpha{% else %}stable{% endif %}</html>";
	...
	fn it_replaces_variable_within_if() {
		...
		let data:HashMap<&str, &str> =
			[("alpha", "true"), ("alpha_str", "holla alpha")].into();
		let rendered = render(template, &data).expect("it should render");
		let expected = r#"
        <html>
        <h1>Hello<h2>
	...
	fn it_performs_nested_conditions() {
		...
		let data:HashMap<&str, &str> =
			[("alpha", "false"), ("v2", "true"), ("alpha_str", "holla alpha")]
				.into();
		let rendered = render(template, &data).expect("it should render");
		let expected = r#"
        <html>
	...
	#[test]
	fn truthy_and_falsy() {
		let template = "<html>Hello {% if beforeDevCommand %}{% \
		                beforeDevCommand %}{% endif %}</html>";
		let data:HashMap<&str, &str> =
			[("beforeDevCommand", "pnpm run")].into();
		let rendered = render(template, &data).expect("it should render");
	...
	#[test]
	fn negated_value() {
		let template = "<html>Hello {% if !name %}world{% else %}{ %name% \
		                }{%endif %}</html>";
		...
		let template = "<html>Hello {% if !render %}world{% else %}{% name \
		                %}{%endif %}</html>";
		let data:HashMap<&str, &str> =
			[("render", "true"), ("name", "tauri")].into();
		let rendered = render(template, &data).expect("it should render");
		assert_eq!(rendered, "<html>Hello tauri</html>");
